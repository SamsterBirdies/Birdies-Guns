...
dofile = "dofile(path .. \"/weapons/sbflamethrower/sbflamethrower.lua\")"
EnergyFireCost = 2000.0
MetalFireCost = 50.0
MinFireSpeed = 6000.0
MaxFireSpeed = 6000.1
base_ReloadTime = 30.0
base_Projectile = "_sbflames"

for x in range (2, iterations + 1):
	newSprite = "Sprites={{Name = \"sbflamethrower-barrel"+str(x)+ "\",States={Normal={Frames={{texture= path .. \"/weapons/sbflamethrower/barrel.tga\",colour={"+str(math.sqrt(((x>>4) & 3)/4.0))+","+str(math.sqrt(((x>>2) & 3)/4.0))+","+str(math.sqrt((x & 3)/4.0))+",1},},mipmap=true,},},Idle=Normal,},},}"
	newRoot = "Root.ChildrenBehind[1].ChildrenBehind[1].Sprite=\"sbflamethrower-barrel"+str(x)+"\""
	...
	file = open("../../weapons/indef/sbflamethrower/_sbflamethrower" + str(x) + ".lua","w")
	file.write(dofile + "\n")
